...
const BusinessCustomer = () => {
  ...
  const [businessProfile, setProfileData] = useState<BusinessProfile>({
    display_name: "",
    gstIn: "",
    phone: "",
    address: "",
  ...
  useEffect(() => setErr(""), [businessProfile]);
  ...
  const handleBusinessProfile = async () => {
    try {
      ...
      if (requiredFields.some((value) => !value)) return setErr("All fields are required");
      if (!GSTRegex.test(businessProfile.gstIn)) return setErr("Invalid GSTIN");
      if (!phoneRegex.test(businessProfile.phone)) return setErr("10 digit phone number is required");
      if (displayNameLength < 3 || displayNameLength > 50) return setErr("Display name should be 3-50 characters");
      if (addressLength < 10 || addressLength > 80) return setErr("Address should be 10-80 characters");
      ...
      if (res.status === 200) {
        ...
        setLoading(false);
        setErr(apiResponse.error || "Unknown error occurred");
      }
    } catch (error) {
      setLoading(false);
      ...
      <div className="flex justify-center items-center h-full">
        ...
        <div className="flex justify-between flex-col-reverse md:flex-row bg-white rounded-b-md md:rounded-md sm:[90%] md:[w-80%] lg:w-[60%] h-full md:h-auto shadow-md: md:shadow-lg shadow-gray-200 overflow-hidden">
          <BusinessCustomerForm
            ...
            loading={loading}
          />
          <div className="md:flex-1 bg-gradient-to-r from-gray-800 via-gray-900 to-black flex justify-center items-center flex-col p-2 gap-2 text-gray-100 h-[18rem] md:h-[25rem] rounded-b-[5rem] md:rounded-l-[6.5rem] md:rounded-br-[0]">
