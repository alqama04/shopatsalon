...
const variants = {
  base: 'p-2 relative rounded-md aspect-square flex justify-center items-center flex-col cursor-pointer   border border-dashed border-gray-400 dark:border-gray-300 transition-colors duration-200 ease-in-out',
      ...
      <div>
        <div className="grid grid-cols-4 gap-2">
          ...
          {value?.map(({ file, progress }, index) => (
            <div
              key={index}
              className={variants.image + " aspect-square h-[100%]"}
            >
              <Image
                height={200}
                width={200}
          ...
          {(!value || value.length < (dropzoneOptions?.maxFiles ?? 0)) && (
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center text-xs text-gray-400">
                ...
                <div className="mt-1">
                  <Button disabled={disabled?disabled:true}>select</Button>
                </div>
              </div>
            </div>
          )}
