...
import { FaCheckCircle,FaRegFile,FaTrashAlt } from "react-icons/fa";
...
const variants = {
  base: 'relative rounded-md p-4 w-80 max-w-[calc(100vw-1rem)] flex justify-center items-center flex-col cursor-pointer border border-dashed border-gray-500 dark:border-gray-400 transition-colors duration-200 ease-in-out',
  active: 'border-2',
  disabled:
              ...
              <div className="flex items-center gap-2 text-gray-500">
                ...
                <div className="flex w-12 justify-end text-xs">
                  {progress === 'PENDING' ? (
                    ...
                    <LuFileWarning className="shrink-0 text-red-600 dark:text-red-400" />
                  ) : progress !== 'COMPLETE' ? (
                    <div>{Math.round(progress)}%</div>
                  ) : (
                    <FaCheckCircle className="shrink-0 text-green-600 dark:text-gray-400" />
                  )}
                </div>
              </div>
              {/* Progress Bar */}
